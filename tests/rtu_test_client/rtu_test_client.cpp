// rtu_test_client.cpp : Defines the entry point for the console application.
//

#include "stdafx.h"
#include "rtu_test_client.h"
#include <windows.h>
#include <commctrl.h>
...
#define ASSERT_TRUE(cond, printf_exp) \
    (cond) ? printf("OK\n") : (printf("\nLine %d: assertion error ", __LINE__), (printf printf_exp))
...
int _tmain(int argc, TCHAR* argv[], TCHAR* envp[])
{
    ...
    ctx = modbus_new_rtu(_T("COM2:"), 115200, _T('N'), 8, 1);
     ...
     */
    Sleep(old_response_to_sec * 1000 + old_response_to_usec/1000);
    modbus_flush(ctx);
    ...
    ctx = modbus_new_rtu(_T("/dev/dummy"), 0, _T('A'), 0, 0);
    ASSERT_TRUE(ctx == NULL && errno == EINVAL, (""));
