...
static ssize_t _modbus_rtu_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    if (ctx_rtu->rts != MODBUS_RTU_RTS_NONE) {
        ...
        if (ctx->debug) {
            mb_fprintf(_T("Sending request using RTS signal\n"));
        }
...
static int _modbus_rtu_receive(modbus_t *ctx, uint8_t *req)
{
    ...
    if (ctx_rtu->confirmation_to_ignore) {
        ...
        rc = 0;
        if (ctx->debug) {
            mb_fprintf(_T("Confirmation to ignore\n"));
        }
    } else {
                                              ...
                                              const uint8_t *rsp, int rsp_length)
{
    ...
    if (req[0] != rsp[0] && req[0] != MODBUS_BROADCAST_ADDRESS) {
        if (ctx->debug) {
            mb_fprintf(_T("The responding slave %d isn't the requested slave %d\n"), rsp[0], req[0]);
        }
        errno = EMBBADSLAVE;
        return -1;
    } else {
                                       ...
                                       const int msg_length)
{
    ...
    if (slave != ctx->slave && slave != MODBUS_BROADCAST_ADDRESS) {
        if (ctx->debug) {
            mb_fprintf(_T("Request for slave %d ignored (not %d)\n"), slave, ctx->slave);
        }
    ...
    if (crc_calculated == crc_received) {
        ...
        if (ctx->debug) {
            mb_fprintf(_T("ERROR CRC received 0x%0X != CRC calculated 0x%0X\n"),
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    if (ctx->debug) {
        mb_fprintf(_T("Opening %s at %d bauds (%c, %d, %d)\n"),
     ...
     * number */
    ctx_rtu->w_ser.fd = CreateFile(ctx_rtu->device,
    ...
    if (ctx_rtu->w_ser.fd == INVALID_HANDLE_VALUE) {
        if (ctx->debug) {
            mb_fprintf(_T("ERROR Can't open the device %s (LastError %d)\n"),
    ...
    if (!GetCommState(ctx_rtu->w_ser.fd, &ctx_rtu->old_dcb)) {
        if (ctx->debug) {
            mb_fprintf(_T("ERROR Error getting configuration (LastError %d)\n"),
                    (int)GetLastError());
        }
    ...
    default:
        dcb.BaudRate = CBR_9600;
        if (ctx->debug) {
            mb_fprintf(_T("WARNING Unknown baud rate %d for %s (B9600 used)\n"),
    ...
    /* Parity */
    if (ctx_rtu->parity == _T('N')) {
        dcb.Parity = NOPARITY;
        dcb.fParity = FALSE;
    } else if (ctx_rtu->parity == _T('E')) {
    ...
    if (!SetCommState(ctx_rtu->w_ser.fd, &dcb)) {
        if (ctx->debug) {
            mb_fprintf(_T("ERROR Error setting new configuration (LastError %d)\n"),
                    (int)GetLastError());
        }
        CloseHandle(ctx_rtu->w_ser.fd);
        ctx_rtu->w_ser.fd = INVALID_HANDLE_VALUE;
    ...
    if (ctx->s == -1) {
        if (ctx->debug) {
            mb_fprintf(_T("ERROR Can't open the device %s (%s)\n",
...
#else
        if (ctx->debug) {
            mb_fprintf(_T("This function isn't supported on your platform\n"));
        }
        errno = ENOTSUP;
        return -1;
#endif
...
static void _modbus_rtu_close(modbus_t *ctx)
{
    ...
    if (!SetCommState(ctx_rtu->w_ser.fd, &ctx_rtu->old_dcb) && ctx->debug) {
        mb_fprintf(_T("ERROR Couldn't revert to configuration (LastError %d)\n"),
                (int)GetLastError());
    }

    if (!CloseHandle(ctx_rtu->w_ser.fd) && ctx->debug) {
        mb_fprintf(_T("ERROR Error while closing handle (LastError %d)\n"),
                (int)GetLastError());
    }
#else
    if (ctx->s != -1) {
                              ...
                              struct timeval *tv, int length_to_read)
{
    ...
    while ((s_rc = select(ctx->s+1, rset, NULL, NULL, tv)) == -1) {
        if (errno == EINTR) {
            if (ctx->debug) {
                mb_fprintf(_T("A non blocked signal was caught\n"));
            }
...
modbus_t* modbus_new_rtu(const TCHAR *device,
                         int baud, TCHAR parity, int data_bit,
                         int stop_bit)
{
    modbus_t *ctx;
    modbus_rtu_t *ctx_rtu;
    ...
    if (device == NULL || *device == 0) {
        mb_fprintf(_T("The device string is empty\n"));
        errno = EINVAL;
        return NULL;
    }
    ...
    /* Check baud argument */
    if (baud == 0) {
        mb_fprintf(_T("The baud rate value must not be zero\n"));
        errno = EINVAL;
        return NULL;
    }
    ...
    /* Device name and \0 */
    ctx_rtu->device = (TCHAR *)malloc((_tcslen(device) + 1) * sizeof(TCHAR));
    _tcscpy(ctx_rtu->device, device);

    ctx_rtu->baud = baud;
    if (parity == _T('N') || parity == _T('E') || parity == _T('O')) {
