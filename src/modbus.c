...
const TCHAR *modbus_strerror(int errnum) {
    switch (errnum) {
    case EMBXILFUN:
        return _T("Illegal function");
    case EMBXILADD:
        return _T("Illegal data address");
    case EMBXILVAL:
        return _T("Illegal data value");
    case EMBXSFAIL:
        return _T("Slave device or server failure");
    case EMBXACK:
        return _T("Acknowledge");
    case EMBXSBUSY:
        return _T("Slave device or server is busy");
    case EMBXNACK:
        return _T("Negative acknowledge");
    case EMBXMEMPAR:
        return _T("Memory parity error");
    case EMBXGPATH:
        return _T("Gateway path unavailable");
    case EMBXGTAR:
        return _T("Target device failed to respond");
    case EMBBADCRC:
    ...
    case EMBMDATA:
        return _T("Too many data");
    case EMBBADSLAVE:
        return _T("Response not from requested slave");
    default:
...
void _error_print(modbus_t *ctx, const TCHAR *context)
{
    if (ctx->debug) {
        mb_fprintf(_T("ERROR %s"), modbus_strerror(errno));
        if (context != NULL) {
            mb_fprintf(_T(": %s\n"), context);
        } else {
            mb_fprintf(_T("\n"));
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    if (ctx->debug) {
        if (msg_type == MSG_INDICATION) {
            mb_fprintf(_T("Waiting for a indication...\n"));
        } else {
            mb_fprintf(_T("Waiting for a confirmation...\n"));
        }
    }
    ...
    while (length_to_read != 0) {
        ...
        if (ctx->debug) {
            int i;
            for (i=0; i < rc; i++)
                mb_fprintf(_T("<%.2X>"), msg[msg_length + i]);
        }
        ...
        if (length_to_read == 0) {
            ...
            case _STEP_META:
                ...
                if ((msg_length + length_to_read) > (int)ctx->backend->max_adu_length) {
                    errno = EMBBADDATA;
                    _error_print(ctx, _T("too many data"));
                    return -1;
                }
                step = _STEP_DATA;
                break;
                              ...
                              uint8_t *rsp, int rsp_length)
{
    ...
    if ((rsp_length == rsp_length_computed ||
        ...
        if (function != req[offset]) {
            if (ctx->debug) {
                mb_fprintf(_T("Received function not corresponding to the request (0x%X != 0x%X)\n"),
                        function, req[offset]);
            }
            if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) {
                _sleep_response_timeout(ctx);
        ...
        if (req_nb_value == rsp_nb_value) {
            ...
            if (ctx->debug) {
                mb_fprintf(_T("Quantity not corresponding to the request (%d != %d)\n"),
        ...
        if (ctx->debug) {
            mb_fprintf(_T("Message length not corresponding to the computed length (%d != %d)\n"),
                 ...
                 int req_length, modbus_mapping_t *mb_mapping)
{
    ...
    case MODBUS_FC_READ_COILS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_BITS < nb) {
            if (ctx->debug) {
                mb_fprintf(_T("Illegal nb of values %d in read_bits (max %d)\n"),
            ...
            if (ctx->debug) {
                mb_fprintf(_T("Illegal data address 0x%0X in read_bits\n"),
    ...
    case MODBUS_FC_READ_DISCRETE_INPUTS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_BITS < nb) {
            if (ctx->debug) {
                mb_fprintf(_T("Illegal nb of values %d in read_input_bits (max %d)\n"),
                        nb, MODBUS_MAX_READ_BITS);
            }
            _sleep_response_timeout(ctx);
            modbus_flush(ctx);
            ...
            if (ctx->debug) {
                mb_fprintf(_T("Illegal data address 0x%0X in read_input_bits\n"),
    ...
    case MODBUS_FC_READ_HOLDING_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_REGISTERS < nb) {
            if (ctx->debug) {
                mb_fprintf(_T("Illegal nb of values %d in read_holding_registers (max %d)\n"),
            ...
            if (ctx->debug) {
                mb_fprintf(_T("Illegal data address 0x%0X in read_registers\n"),
    ...
    case MODBUS_FC_READ_INPUT_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_REGISTERS < nb) {
            if (ctx->debug) {
                mb_fprintf(_T("Illegal number of values %d in read_input_registers (max %d)\n"),
                        nb, MODBUS_MAX_READ_REGISTERS);
            }
            _sleep_response_timeout(ctx);
            modbus_flush(ctx);
            ...
            if (ctx->debug) {
                mb_fprintf(_T("Illegal data address 0x%0X in read_input_registers\n"),
    ...
    case MODBUS_FC_WRITE_SINGLE_COIL: {
        ...
        if (address < mb_mapping->offset_bits || addr >= mb_mapping->nb_bits) {
            if (ctx->debug) {
                mb_fprintf(_T("Illegal data address 0x%0X in write_bit\n"), address);
            }
            rsp_length = response_exception(
                ctx, &sft,
                MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp);
            ...
            if (data == 0xFF00 || data == 0x0) {
                ...
                if (ctx->debug) {
                    mb_fprintf(_T("Illegal data value 0x%0X in write_bit request at address %0X\n"),
                            data, address);
                }
                rsp_length = response_exception(
                    ctx, &sft,
    ...
    case MODBUS_FC_WRITE_SINGLE_REGISTER: {
        ...
        if (address < mb_mapping->offset_registers || addr >= mb_mapping->nb_registers) {
            if (ctx->debug) {
                mb_fprintf(_T("Illegal data address 0x%0X in write_register\n"), address);
            }
            rsp_length = response_exception(
                ctx, &sft,
                MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp);
    ...
    case MODBUS_FC_WRITE_MULTIPLE_COILS: {
        ...
        if (nb < 1 || MODBUS_MAX_WRITE_BITS < nb) {
            if (ctx->debug) {
                mb_fprintf(_T("Illegal number of values %d in write_bits (max %d)\n"),
            ...
            if (ctx->debug) {
                mb_fprintf(_T("Illegal data address 0x%0X in write_bits\n"),
                        address < mb_mapping->offset_bits ? address : address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case MODBUS_FC_WRITE_MULTIPLE_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_WRITE_REGISTERS < nb) {
            if (ctx->debug) {
                mb_fprintf(_T("Illegal number of values %d in write_registers (max %d)\n"),
            ...
            if (ctx->debug) {
                mb_fprintf(_T("Illegal data address 0x%0X in write_registers\n"),
                        address < mb_mapping->offset_registers ? address : address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case MODBUS_FC_READ_EXCEPTION_STATUS:
        if (ctx->debug) {
            mb_fprintf(_T("FIXME Not implemented\n"));
        }
        errno = ENOPROTOOPT;
        return -1;
        break;
    ...
    case MODBUS_FC_WRITE_AND_READ_REGISTERS: {
        ...
        if (nb_write < 1 || MODBUS_MAX_WR_WRITE_REGISTERS < nb_write ||
            ...
            if (ctx->debug) {
                mb_fprintf(_T("Illegal nb of values (W%d, R%d) in write_and_read_registers (max W%d, R%d)\n"),
                        nb_write, nb,
            ...
            if (ctx->debug) {
                mb_fprintf(_T("Illegal data read address 0x%0X or write address 0x%0X write_and_read_registers\n"),
                        address < mb_mapping->offset_registers ? address : address + nb,
...
int modbus_read_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest)
{
    ...
    if (nb > MODBUS_MAX_READ_BITS) {
        if (ctx->debug) {
            mb_fprintf(_T("ERROR Too many bits requested (%d > %d)\n",
                    nb, MODBUS_MAX_READ_BITS));
        }
        errno = EMBMDATA;
        return -1;
...
int modbus_read_input_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest)
{
    ...
    if (nb > MODBUS_MAX_READ_BITS) {
        if (ctx->debug) {
            mb_fprintf(_T("ERROR Too many discrete inputs requested (%d > %d)\n"),
                    nb, MODBUS_MAX_READ_BITS);
        }
        errno = EMBMDATA;
        return -1;
                                ...
                                uint16_t *dest)
{
    ...
    if (nb > MODBUS_MAX_READ_REGISTERS) {
        mb_fprintf(_T("ERROR Too many input registers requested (%d > %d)\n",
                nb, MODBUS_MAX_READ_REGISTERS));
        errno = EMBMDATA;
        return -1;
    }
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src)
{
    ...
    if (nb > MODBUS_MAX_WRITE_BITS) {
        if (ctx->debug) {
            mb_fprintf(_T("ERROR Writing too many bits (%d > %d)\n",
                    nb, MODBUS_MAX_WRITE_BITS));
        }
        errno = EMBMDATA;
        return -1;
...

{
    ...
    if (write_nb > MODBUS_MAX_WR_WRITE_REGISTERS) {
        if (ctx->debug) {
            mb_fprintf(_T("ERROR Too many registers to write (%d > %d)\n",
 ...
 */
size_t strlcpy(TCHAR *dest, const TCHAR *src, size_t dest_size)
{
    register TCHAR *d = dest;
    register const TCHAR *s = src;
